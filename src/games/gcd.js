...
const getGcd = (first, second) => {
  ...
  const getDivisor = (divisor, greatestDivisor) => {
    ...
    return (first % divisor === 0 && second % divisor === 0)
      ? getDivisor(divisor + 1, divisor)
      : getDivisor(divisor + 1, greatestDivisor);
  };
  return getDivisor(1, 0);
};
const getQuestionAnswer = () => {
  const first = getRandomValue(1, 100);
  const second = getRandomValue(1, 100);
